...
SELECT orders.customerNumber AS customerNumbers, SUM((orderdetails.priceEach-products.buyPrice)*orderdetails.quantityOrdered) AS profit, SUM((orderdetails.priceEach-products.buyPrice)*orderdetails.quantityOrdered)over() AS totalProfit FROM orderdetails
JOIN orders ON orders.orderNumber = orderdetails.orderNumber
JOIN products ON products.productCode = orderdetails.productCode
GROUP BY orders.orderNumber;

SELECT customerNumbers, SUM(profit), SUM(profit/totalProfit)  FROM profits
...
SELECT customers.salesRepEmployeeNumber, SUM(orderdetails.priceEach*orderdetails.quantityOrdered) AS revenue FROM customers
JOIN orders ON customers.customerNumber = orders.customerNumber
JOIN orderdetails ON orderdetails.orderNumber = orders.orderNumber
...
SELECT customers.salesRepEmployeeNumber, SUM((orderdetails.priceEach-products.buyPrice)*orderdetails.quantityOrdered) AS profit FROM customers
JOIN orders ON customers.customerNumber = orders.customerNumber
JOIN orderdetails ON orderdetails.orderNumber = orders.orderNumber
JOIN products ON products.productCode = orderdetails.productCode
GROUP BY customers.salesRepEmployeeNumber
...
SELECT orderdetails.productCode, products.productName, SUM(orderdetails.priceEach*orderdetails.quantityOrdered) AS revenue FROM orderdetails
JOIN products ON products.productCode = orderdetails.productCode
GROUP BY products.productCode
ORDER BY products.productName;

-- 24 Compute the profit generated by each product line, sorted by profit descending.
SELECT products.productLine, SUM((orderdetails.priceEach-products.buyPrice)*orderdetails.quantityOrdered) AS profit FROM products
JOIN orderdetails ON products.productCode = orderdetails.productCode
...
DROP VIEW y2003;
CREATE VIEW y2003 AS
SELECT orderdetails.productCode AS ProductCode, SUM(quantityOrdered*priceEach) AS Sales03 FROM orderdetails
JOIN orders ON orders.orderNumber = orderdetails.orderNumber
...
SELECT orderdetails.productCode AS ProductCode, SUM(quantityOrdered*priceEach) AS Sales04 FROM orderdetails
JOIN orders ON orders.orderNumber = orderdetails.orderNumber
WHERE year(orders.orderDate) = 2004
GROUP BY orderdetails.productCode;

SELECT y2003.ProductCode, y2003.Sales03, y2004.Sales04, y2003.Sales03/y2004.Sales04 AS ratio FROM y2003
JOIN y2004 ON y2004.ProductCode = y2003.ProductCode;

-- 26 Compute the ratio of payments for each customer for 2003 versus 2004.
DROP VIEW p2003;
CREATE VIEW p2003 AS
SELECT customerNumber, SUM(amount) AS p03 FROM payments
WHERE year(paymentDate) = 2003
GROUP BY customerNumber;
 
DROP VIEW p2004;
CREATE VIEW p2004 AS
SELECT customerNumber, SUM(amount) AS p04 FROM payments
WHERE year(paymentDate) = 2004
GROUP BY customerNumber;

SELECT p2003.customerNumber AS customerNumber, p2003.p03, p2004.p04, p2003.p03/p2004.p04 AS ratio FROM p2003
JOIN p2004 ON p2003.customerNumber = p2004.customerNumber;

-- 27 Find the products sold in 2003 but not 2004.
SELECT productCode FROM orderdetails
JOIN orders ON orders.orderNumber = orderdetails.orderNumber
WHERE orders.orderNumber NOT IN (SELECT orderNumber FROM orders WHERE year(orderDate)= 2004)
AND orders.orderNumber IN (SELECT orderNumber FROM orders WHERE year(orderDate)= 2003);
